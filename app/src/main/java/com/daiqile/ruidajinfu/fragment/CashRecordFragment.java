...
import com.daiqile.ruidajinfu.adapter.CashRecordAdapter;
...
public class CashRecordFragment extends BaseFragment {
    ...
    public void init() {
        ...
        adapter = new CashRecordAdapter(mActivity);
    ...
    private void getCashRecord(final boolean isRefresh) {
        Map<String, String> map = new HashMap<>();
        map.put("user_id", String.valueOf(application.mUser.getId()));
        map.put("page", pageNum + "");
        application.apiService.getCashRecord(map)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<CashRecord>() {
                    @Override
                    ...
                    public void onNext(CashRecord cashRecord) {
                        if (isRefresh) {
                            adapter.clear();
                            adapter.addAll(cashRecord.getList());
                        } else {
                            adapter.addAll(cashRecord.getList());
                        }
